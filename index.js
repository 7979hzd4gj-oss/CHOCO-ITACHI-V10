...
const runtime = (s)=>{s=Number(s);let d=Math.floor(s/(3600*24)),h=Math.floor(s%86400/3600),m=Math.floor(s%3600/60);return `${d>0?d+"j ":""}${h}h ${m}m`};
...
⚪ dare • truth • roll • flip • slot • flirt

『 ADMIN GROUP 』
⚪ open • close • ban • kick • warn
⚪ promote • demote • mute • unmute
⚪ delete • clear • tagall • tag • hidetag
⚪ add • remove • link • revoke
⚪ setgname • setgpp • setdesc
⚪ getdesc • getgpp • admins • members
⚪ warnings • resetwarn • poll • approve
⚪ invite • gstatus • welcome • goodbye
⚪ setwelcome • setgoodbye • purge • kickall

『 PROTECTION 22 』
⚪ antilink • antibadword • antibot
⚪ antisticker • antileave • antimention
⚪ antitag • anticall • antidelete
⚪ antipurge • antimarabou • antistatut
⚪ antifake • antispam • antiviewonce
⚪ antigroup • antivoice • antifile
⚪ antishare • antiflood • antiedit
⚪ antichannel

『 DOWNLOAD 』
⚪ play • song • video • ytmp3 • ytmp4
⚪ yts • tiktok • insta • fb • mediafire
⚪ apk • spotify • vv
...
case "tourl":case "toUrl": {try{let med=qmsg?{message:qmsg}:m; let b=await downloadMediaMessage(med,'buffer',{},{}); await send("Media recu "+b.length+" bytes")}catch{await send("Reponds a une image avec.tourl")} break;}
...
case "tiktok": {await send("Tiktok DL: "+q+" (ajoute API si tu veux)"); break;}
case "insta":case "fb":case "mediafire":case "apk":case "spotify":case "vv": {await send(`${cmd.toUpperCase()} DL: Reçu ${q}`); break;}
